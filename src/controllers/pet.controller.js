...
const ACHIEVEMENT_DEFINITIONS = [
    ...
    { id: 'rich_1000',       label: '💰 Đại Gia',                description: 'Tích lũy tổng cộng 1000 Love Coins',   target: 1000, coins: 100 },
...
async function checkAndUpdateAchievements(coupleRoomId) {
    try {
        // Fetch current stats
        const [petRes, actionsRes, expeditionsRes, purchasesRes, coinsRes] = await Promise.all([
            ...
            query(`SELECT COUNT(*)::int AS cnt FROM coin_reward_logs WHERE couple_room_id = $1 AND reward_type = 'shop_purchase'`, [coupleRoomId]),
        ...
        const progressMap = {
            ...
            balance_7:       0, // Complex calculation, simplified for now
            pet_age_30:      petAge,
            pet_age_100:     petAge,
            shopper_20:      totalPurchases,
        };

        // Yin-Yang balance check (simplified: if no penalty currently, count as balanced)
        const totalCare = pet.user1_interactions + pet.user2_interactions;
        if (totalCare > 20) {
            const ratio = Math.max(pet.user1_interactions, pet.user2_interactions) / totalCare;
            progressMap.balance_7 = ratio <= 0.65 ? 7 : 0; // Simplified
        }
        ...
        for (const def of ACHIEVEMENT_DEFINITIONS) {
            ...
            // Check if newly unlocked (unlocked but coins not yet rewarded)
            if (row && row.unlocked && row.coins_rewarded === 0 && progress >= def.target) {
                // Award coins
                await query(
                    `UPDATE pet_achievements SET coins_rewarded = $3 WHERE couple_room_id = $1 AND achievement_id = $2`,
...
async function checkDailyReward(coupleRoomId, userId) {
    try {
        ...
        const existing = await query(
            `SELECT id FROM coin_reward_logs WHERE couple_room_id = $1 AND reward_type = 'daily_login' AND reward_key = $2`,
            [coupleRoomId, rewardKey]
        );

        if (existing.rows.length > 0) return null; // Already claimed
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            const checkKey = `daily_${checkDate.toISOString().slice(0, 10)}`;
        ...
        logger.info(`[Pet] Daily reward: room=${coupleRoomId}, streak=${streak + 1}, coins=${coinsAwarded}`);

        return {
            coinsAwarded,
            streak: streak + 1,
            isWeeklyBonus: streak >= 6,
        };
    } catch (err) {
