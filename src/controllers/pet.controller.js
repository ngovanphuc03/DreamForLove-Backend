...
async function performAction(req, res, next) {
    try {
        ...
        const result = await transaction(async (client) => {
            ...
            const newTotalXp = pet.total_love_xp + xpAwarded;
            ...
            if (newTotalXp >= LEVEL_THRESHOLDS[oldLevel] && oldLevel < 8) {
                // To evolve, they must have an evolution stone and actively trigger it.
                // For now, if they reach the threshold, we lock xp or flag it.
                requiresEvolutionStone = true;
            }

            if (requiresEvolutionStone && newTotalXp > LEVEL_THRESHOLDS[oldLevel]) {
                 // Prevent gaining XP beyond limit if blocked
                 xpAwarded = 0;
            }
        ...
        const { pet, xpAwarded, evolved, oldLevel, newLevel,
                myActionsToday, partnerActionsToday, cooperativeBonus } = result;
...
async function buyShopItem(req, res, next) {
    try {
        ...
        const result = await transaction(async (client) => {
            const roomRes = await client.query('SELECT love_coins FROM couple_rooms WHERE id = $1', [roomId]);
            const coins = roomRes.rows[0].love_coins;
            ...
            // Deduct coins
            await client.query('UPDATE couple_rooms SET love_coins = love_coins - $2 WHERE id = $1', [roomId, totalPrice]);
            ...
            // Fetch new inventory and new coins
            const newCoinsRes = await client.query('SELECT love_coins FROM couple_rooms WHERE id = $1', [roomId]);
            const newInventoryRes = await client.query('SELECT item_id, quantity FROM pet_inventory WHERE couple_room_id = $1', [roomId]);

            return {
                loveCoins: newCoinsRes.rows[0].love_coins,
